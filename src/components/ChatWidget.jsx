...
//       //   : '/chat';
//             // Use explicit backend URL for debugging
...
//         context: selectedText || null  // Include selected text if available
//       };
...
const ChatWidget = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const sendText = inputValue; // store before clearing
    setInputValue("");
    setIsLoading(true);
    ...
    try {
      ...
      const botMessage = {
        role: "bot",
        content: data.response || "No reply received.",
      };
      ...
      setMessages((prev) => [
        ...prev,
        { role: "bot", content: "Error connecting to server: " + err.message },
      ]);
    }
  ...
  return (
    <>
      {isOpen ? (
        ...
        <button
          className={`chat-toggle ${colorMode}`}
